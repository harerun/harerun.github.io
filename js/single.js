...
function single_load_data(callback) {
    app_load_json(single, '/database/single.json', callback,
        function(single_data) { return single_data.reverse(); });
}

function load_single() {
    document.title = app_i18n.title_single;
    header_set_item('nav-single');
    // Load the single data first.
    single_load_data(function() {
